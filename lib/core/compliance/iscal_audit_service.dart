...
class IscalAuditService {
  ...
  Future<void> sealFacture(
    Facture facture, {
    String action = 'PAYMENT_SEAL',
  }) async {
    ...
    await _remoteStorage.saveRaw(
      _logCollection,
      '${facture.id}_${DateTime.now().millisecondsSinceEpoch}',
      {
        'action': action,
  ...
  Future<bool> isPeriodClosed(String companyId, DateTime date) async {
    ...
    final List<Map<String, dynamic>> closures = await _remoteStorage
        .watchCollectionRaw(
          _closureCollection,
  ...
  Future<void> generateClosure(
    String companyId,
    ClosurePeriod periodType,
    DateTime date,
  ) async {
