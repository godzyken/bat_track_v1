...
void main() {
  group('JsonModel Mixin', () {
    test('parseDateTime should handle various input formats', () {
      // String ISO
      final date1 = JsonModel.parseDateTime('2024-01-15T10:30:00.000Z');
      expect(date1, isNotNull);
      expect(date1!.year, 2024);

      // DateTime object
      final now = DateTime.now();
      final date2 = JsonModel.parseDateTime(now);
      expect(date2, equals(now));

      // Null input
      final date3 = JsonModel.parseDateTime(null);
      expect(date3, isNull);

      // Invalid string
      final date4 = JsonModel.parseDateTime('invalid');
      expect(date4, isNull);
    });

    test('parseDateTimeNonNull should throw on null/invalid input', () {
      expect(
        () => JsonModel.parseDateTimeNonNull(null),
        throwsA(isA<FormatException>()),
      );
      expect(
        () => JsonModel.parseDateTimeNonNull('invalid'),
        throwsA(isA<FormatException>()),
      );
    });
    ...
    test('toJsonDateTime should convert DateTime to ISO string', () {
      ...
      final jsonString = JsonModel.toJsonDateTime(date);
      expect(jsonString, equals(date.toIso8601String()));

      final nullResult = JsonModel.toJsonDateTime(null);
